...
from .modules import MoeMLP_DiffMoE as MoeMLP
...
class SparseMoEBlock(nn.Module):
    ...
    def __init__(self, experts, hidden_dim, num_experts, n_shared_experts=0, capacity=2, mlp_ratio=4.0,
        ...
        if use_diff_expert:
            self.diff_expert = MoeMLP(hidden_size=hidden_size, intermediate_size=hidden_dim * mlp_ratio)
    ...
    def forward_train(self, x):
        ...
        if self.use_diff_expert:
            diff_expert_output = self.diff_expert(identity)
            x_out = x_out - diff_expert_output
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DiTBlock(hidden_size, num_heads // (1 if use_moe_flag[_] else 1), head_dim=head_dim, mlp_ratio=mlp_ratio, qk_norm=qk_norm, 
